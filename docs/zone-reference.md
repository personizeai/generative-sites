...
| `data-gs-guidelines` | Optional | Comma-separated guideline names to apply. Set on script tag (site-wide) and/or individual zones (additive). |
...
`document.currentScript` is `null` when using React's `<Script>` component or dynamic injection. gs.js handles this by searching the DOM for `script[data-key]`.
...
useEffect(() => {
  ...
  s.src = 'https://gs.personize.ai/gs.js';
  s.setAttribute('data-key', 'pk_live_SITE_KEY');
  s.async = true;
...
After zones finish streaming, gs.js automatically memorizes each generated zone's text back as property values. On return visits, property zones load these stored values instantly — no AI generation.

- **First visit:** Zones generated by AI → auto-memorized as properties
- **Return visit:** Property zones serve stored values instantly

Requires `data-gs-identify` on the page and an identified visitor. No extra configuration.
